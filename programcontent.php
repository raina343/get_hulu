...
<th class="search"><table class="searchhead"><tr class="search"><th class="search"><label class="unsorted pointer" onclick="getlist('sort','is_subscriber_only','<?echo $otherdir;?>')" title="Sort by type">Type</label></th></tr></table></th>
...
if ($_POST['Search']=="true"){
	$dbQuery = "SELECT * FROM ProgramData WHERE (show_name LIKE '%".mysql_real_escape_string($_POST['value'])."%' OR title LIKE '%".mysql_real_escape_string($_POST['value'])."%') AND (is_subscriber_only='0' AND expires_at >'".date(c)."' OR expires_at='0000-00-00') ORDER BY ".$_POST['SortBy']." ".$_POST['direction'];
}else {
	$dbQuery = "SELECT * FROM ProgramData WHERE is_subscriber_only='0' AND expires_at >'".date(c)."' OR expires_at='0000-00-00' ORDER BY ".$_POST['SortBy']." ".$_POST['direction'];
}
...
print '<td class="search"><label class="search" title="Click for full info">'.$row['is_subscriber_only'].'</label></td>';
